...
export default function WeatherUiToday(props: weatherDataProps) {
  ...
  function detectWidthandHeight() {
    setgetwidth(window.innerWidth);
    setgetheight(window.innerHeight);
    if (window.innerWidth > 522 && loadmore == true) {
      setloadmore(false);
    } else if (window.innerWidth > 522 && loadmore == true) {
      setloadmore(false);
      setgetfetch(true);
    } else if (window.innerWidth > 641 && loadmore == true) {
      setloadmore(false);
      setgetfetch(false);
    }
  }
      ...
      {props.loading ? (
        <>
          ...
          <div className="loadingdiv">
            {' '}
            <img
              src={require('../images/Ripple.gif')}
              className="loadingimage"
            />
          </div>
          <div className="loadingdiv2">
            {' '}
            <h1 className="loading cssanimation leFadeIn random">
              {' '}
              Loading...
            </h1>{' '}
          </div>
        ...
        <>
            ...
            >
              ...
              {props.country ? (
                <div className="datediv">
                  <p className="date">
                    {' '}
                    {getTime.toString().split(' ').slice(0, 4).join(' ') +
                      ',' +
                      ' ' +
                      getTime.getHours() +
                      ':' +
                ...
                <Tippy
                  content={
                    <>
                      ...
                      <div className="tippyhumidcontainer">
                        {' '}
                        Humidity: {props.humidity}%
                      </div>
                      <div> Windspeed {props.windSpeed}km/h</div>
                    </>
                  }
                >
              ...
              <div className="humidityandwind">
                {props.city ? (
                  <span className="humiditydata">{props.humidity}%</span>
                ) : (
                  ''
                )}
                {props.city ? (
                  <span className="winddata">{props.windSpeed}km/h</span>
                ) : (
                  ''
                )}
              </div>
          ...
          {props.city ? (
            <div className="between">
              <Tippy
                content={
                  <div className="tippycontainer">
                    {' '}
                    Click to toggle between More Data{' '}
                  </div>
                }
              >
                <div
                  className={
                    getmouse
                      ? 'arrowdiv cssanimation fadeIn infinite'
                      : 'arrowdiv'
                  }
          ...
          <div
            className={
              props.city
                ? 'customeffectminiweathercontainer cssanimation blurInRight '
                : 'miniweathercontainer cssanimation blurOut'
            }
          >
            {loadmore ? (
              ''
              ...
              <Nextweather
                tomorrowWeatherDesc={props.tomorrowWeatherDesc}
                country={props.country}
                tomorrowTemp={props.tomorrowTemp}
                next_Weather={props.next_Weather}
                tomorrowIcon={props.tomorrowIcon}
              />
            )}
            {loadmore ? (
              ''
              ...
              <Nextweathertwo
                twoDayWeatherDesc={props.twoDayWeatherDesc}
                twoDayIcon={props.twoDayIcon}
                twoDayTemp={props.twoDayTemp}
                twoDayWeather={props.twoDayWeather}
                country={props.country}
              />
            )}
            <Nextweatherthree
              threeDayWeatherDesc={props.threeDayWeatherDesc}
              country={props.country}
              threeDayWeather={props.threeDayWeather}
              threeDayTemp={props.threeDayTemp}
              threeDayIcon={props.threeDayIcon}
            />
            {getwidth > 522 || loadmore ? (
              <Nextweatherfour
                fourDayWeatherDesc={props.fourDayWeatherDesc}
                country={props.country}
                fourDayWeather={props.fourDayWeather}
                fourDayTemp={props.fourDayTemp}
                fourDayIcon={props.fourDayIcon}
              />
            ) : (
              ''
            )}
            {getwidth > 641 || loadmore ? (
              <Nextweatherfive
                fiveDayWeatherDesctwo={props.fiveDayWeatherDesctwo}
                setFiveDayWeatherDesc={props.setFiveDayWeatherDesc}
                fiveDayWeatherDesc={props.fiveDayWeatherDesc}
                country={props.country}
                fiveDayWeather={props.fiveDayWeather}
                fiveDayTemp={props.fiveDayTemp}
                fiveDayIcon={props.fiveDayIcon}
              />
            ) : (
              ''
            )}
